...
const ComplaintList = ({ complaints, setComplaints, setEditingComplaint }) => {
  ...
  const BASE = axiosInstance.defaults.baseURL; // e.g., http://localhost:5001

  const handleUpload = async (complaint, files) => {
    // type check & count check
            ...
            <input
              ...
              onChange={(e) => {
                if (e.target.files?.length) {
                  ...
                  e.target.value = ''; // reset input so same files can be picked again if needed
                }
              }}
